...
func _physics_process(delta: float) -> void:
	...
	if ((Input.is_action_just_released("leftone") or Input.is_action_just_released("rightone")) or (Input.is_action_just_pressed("jumpone") and not satOn)) and inputCount > 0:
		inputCount -= 1
